...
var Map = function Map(selector, options) {
  options = options || {};
...
function fetchAndDrawNodes(isFluidMap) {
  var self = this;
  request.get(config.musicMap.endpoint + '/artists.json?fluid=' + isFluidMap, function(data) {
    var artists = data.body.artists;
    var linkData = [];

    var nodeData = _.map(artists, function(artist) {
      return {
    ...
    _.each(artists, function(artist, sourceArtistIndex) {
      _.each(artist.similar, function(similarArtist) {
        var similarArtistIndex = _.indexOf(_.map(artists, _.createCallback('echonestId')), similarArtist.echonestId);
